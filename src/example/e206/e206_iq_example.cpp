...
{
    std::cout
        << "Usage: e206_iq_example [options]\n"
        << "  --addr <ip>               device IP, default 192.168.1.11\n"
        ...
        << "  --tx-atten <idx>          TX attenuation, default 40\n"
        ...
        << "  --no-tx-pacing            disable TX pacing for stress testing\n"
        << "  --help                    show this help\n";
}
...
void parse_args(Config& cfg, int argc, char** argv)
{
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--tx-pacing") {
            cfg.tx_pacing = true;
        } else if (arg == "--no-tx-pacing") {
            cfg.tx_pacing = false;
        } else if (arg == "--help") {
            print_usage();
            std::exit(0);
...
void print_config(const Config& cfg)
{
    std::cout << "E206 IQ example\n"
              ...
              << "  tx pacing     : " << (cfg.tx_pacing ? "on" : "off") << "\n\n";
}

void configure_device(E206Impl& device, const Config& cfg)
{
...
TxStats run_tx(E206Impl& device,
               ...
               std::atomic<bool>& stop)
{
    ...
    while (!stop.load()) {
        ...
        if (cfg.tx_pacing && sent > 0) {
            const double sample_interval_sec =
                static_cast<double>(sent) / std::max<uint32_t>(cfg.sample_rate_hz, 1u);
            next_tx_time += std::chrono::duration_cast<std::chrono::steady_clock::duration>(
                std::chrono::duration<double>(sample_interval_sec));

            const auto now = std::chrono::steady_clock::now();
            if (next_tx_time > now) {
                std::this_thread::sleep_until(next_tx_time);
            } else if (now - next_tx_time > std::chrono::milliseconds(100)) {
                next_tx_time = now;
            }
        }
    }
...
int main(int argc, char** argv)
{
    try {
        ...
        std::cerr << "e206_iq_example failed: " << ex.what() << '\n';
        return 1;
    }
}
