...
struct Config {
    ...
    std::string output = "e100_record.cs16";
...
{
    std::cout
        << "Usage: e100_record_iq_test [options]\n"
        << "  --addr <ip>              device IP, default 192.168.1.10\n"
        << "  --output <path>          output cs16 file, default e100_record.cs16\n"
...
void parse_args(Config& cfg, int argc, char** argv)
{
    ...
    if (cfg.record_bytes < kRecordLengthMinBytes ||
        ...
        throw std::runtime_error("E100 record length must be 4..240 MiB and a multiple of 4 MiB");
    }
...
void configure_rf(E100Impl& device, const Config& cfg)
{
...
void wait_record_done(E100Impl& device, const Config& cfg)
{
    const auto deadline =
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if ((status & E100Impl::RECORD_STATUS_ERROR) != 0u) {
            throw std::runtime_error("FPGA IQ record reported ERROR");
        }
        if ((status & E100Impl::RECORD_STATUS_DONE) != 0u) {
            return;
        }
...
void read_record_to_file(E100Impl& device, const Config& cfg)
{
...
void run_warmup_record(E100Impl& device, const Config& cfg)
{
    if (cfg.warmup_bytes == 0u) {
        return;
    }
...
int main(int argc, char** argv)
{
    try {
        ...
        std::cerr << "e100_record_iq_test failed: " << ex.what() << '\n';
        return 1;
    }
}
